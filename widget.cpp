...
Widget::Widget(QWidget *parent) : QWidget(parent), ui(new Ui::Widget){
    ...
    player2_dir=new QLibrary("player2_dir");//TODO add to settings
    player2_url=new QLibrary("player2_url");//TODO add to settings
    player2_help=new QLibrary("player2_help");//TODO add to settings
    player2_readme=new QLibrary("player2_readme");//TODO add to settings
    player2_playlist=new QLibrary("player2_playlist");//TODO add to settings
    player2_media_info=new QLibrary("player2_media_info");//TODO add to settings
...
void Widget::dropEvent(QDropEvent *drop){
    ...
    if(mime->hasUrls()) {
#ifdef DEBUG
        qDebug()<<(tr("drop event "));
#endif
        for(QUrl _url : mime->urls()) {
#ifdef DEBUG
            qDebug()<<(QString(tr("add url %1")).arg(_url.path()));
#endif
...
void Widget::create_widget(){
    ...
    player2_dir->load();
#ifdef DEBUG
    qDebug()<<(QString("load plugin %1: %2").arg("player2_dir",player2_dir->isLoaded()?"true":"false"));
#endif
    player2_url->load();
#ifdef DEBUG
    qDebug()<<(QString("load plugin %1: %2").arg("player2_url",player2_url->isLoaded()?"true":"false"));
#endif
    player2_help->load();
#ifdef DEBUG
    qDebug()<<(QString("load plugin %1: %2").arg("player2_help",player2_help->isLoaded()?"true":"false"));
#endif
    player2_readme->load();
#ifdef DEBUG
    qDebug()<<(QString("load plugin %1: %2").arg("player2_readme",player2_readme->isLoaded()?"true":"false"));
#endif
    player2_playlist->load();
#ifdef DEBUG
    qDebug()<<(QString("load plugin %1: %2").arg("player2_playlist",player2_playlist->isLoaded()?"true":"false"));
#endif
    player2_media_info->load();
#ifdef DEBUG
    qDebug()<<(QString("load plugin %1: %2").arg("player2_media_info",player2_media_info->isLoaded()?"true":"false"));
#endif
    //plugin dir
    ...
    //END LOAD PLUGINS
    //	readme=new class_readme(QLocale::system().name(),settings->SETTINGS_FILE->path);
    //	readme->create();
...
void Widget::check_settings(){
    ...
    if(file_time_change_old!=last_modified){
        ...
        settings->re_read();
#ifdef DEBUG
        qDebug()<<tr("start apply settings");
#endif
        timer->setInterval(settings->UPDATE->info_music.toInt());
#ifdef DEBUG
        qDebug()<<tr("recreate readme");
#endif
        readme_create(settings->SETTINGS_FILE->path);
#ifdef DEBUG
        qDebug()<<tr("set style sheet");
#endif
        this->setStyleSheet("background-color:"+settings->WIDGET->background_color);
#ifdef DEBUG
        qDebug()<<tr("set font button");
#endif
        font_button->setFamily(settings->BUTTON->font_family);
        font_button->setPointSize(settings->BUTTON->font_size.toInt());
        font_button->setWeight(settings->BUTTON->font_weight.toInt());
#ifdef DEBUG
        qDebug()<<tr("set font list");
#endif
        font_list_widget->setFamily(settings->LIST->font_family);
        font_list_widget->setPointSize(settings->LIST->font_size.toInt());
        font_list_widget->setWeight(settings->LIST->font_weight.toInt());
#ifdef DEBUG
        qDebug()<<tr("set font property media");
#endif
        font_property_media->setFamily(settings->PROPERTY_MEDIA->font_family);
        font_property_media->setPointSize(settings->PROPERTY_MEDIA->font_size.toInt());
        font_property_media->setWeight(settings->PROPERTY_MEDIA->font_weight.toInt());
        ...
        ui->listWidget->item(index)->setBackgroundColor(QColor(settings->LIST->cur_mus_background_color));
...
void Widget::showWidgetWithSlider(){
    if(show_widget_with_slider) {
        ...
        ui->listWidget->setFixedHeight(settings->PLAYLIST->check_widgetHeight.toInt()-ui->listWidget->y()-5);
        ui->listWidget->item(index)->setBackgroundColor(QColor(settings->LIST->cur_mus_background_color));
        ui->listWidget->item(index)->setTextColor(QColor("lime"));                                 /* / TODO add to settings */
        ui->property_media->setFixedHeight(settings->PLAYLIST->check_widgetHeight.toInt()-ui->property_media->y()-5);
        ui->property_media->setFixedWidth(settings->WIDGET->press_info_but_width.toInt()-ui->property_media->x()-5);
...
#ifdef DEBUG
        qDebug()<<(tr("show widget with slider"));
#endif
    }
}
...
bool Widget::updatePlayList(const int add_path){
    ...
    if(playlistChange()) {
        for(int i=playlistCount; i<playlistLength; i++) {
            ...
            ui->listWidget->item(i)->setTextColor(QColor("lime")); /* / TODO add to settings */
        }
        settings->FILE->path=url_info(playlist_list.last()).dir().absolutePath().trimmed();
#ifdef DEBUG
        qDebug()<<(tr("update playlist"));
#endif
        return true;
    }
    return false;
...
#ifdef DEBUG
    qDebug()<<"check playlist change";
#endif
    playlistCount=ui->listWidget->count();
    playlistLength=playlist_list.length();
#ifdef DEBUG
    qDebug()<<QString("return %1").arg(playlistLength!=playlistCount?"true":"false");
#endif
    return playlistLength!=playlistCount;
}
...
void Widget::nexttitle(QMediaContent media){
    if(musicClicked) {
        ...
        ui->listWidget->item(index)->setBackgroundColor(QColor(settings->LIST->prev_mus_background_color));
        ui->listWidget->item(index)->setTextColor(QColor("lime")); /* / TODO add to settings */
    ...
    this->setWindowTitle(QString("%1").arg(ui->listWidget->item(index)->text().trimmed()));
}
 ...
 */
void Widget::open_network_music(){ /* TODO MOVE TO FORMS */
...
void Widget::clickedAudio(QListWidgetItem *item){
    ...
    for(int i=0; i<playlistCount; i++)
        if(ui->listWidget->item(i)->text()==item->text()) index=i;
    playlist->setCurrentIndex(index);
    ui->listWidget->item(index)->setBackgroundColor(QColor(settings->LIST->cur_mus_background_color));
    ui->listWidget->item(index)->setTextColor(QColor("lime"));                 /* / TODO add to settings */
    musicPaused=false;
...
Widget::~Widget(){
    mediaPlayer->stop();
#ifdef DEBUG
    qDebug()<<(tr("stop mediaPlayer"));
#endif
    settings->~class_settings();
#ifdef DEBUG
    qDebug()<<(tr("destruct settings"));
#endif
    if(player2_dir->unload()){
#ifdef DEBUG
        qDebug()<<(tr("unload player2_dir"));
#endif
    }
    if(player2_url->unload()){
#ifdef DEBUG
        qDebug()<<(tr("unload player2_url"));
#endif
    }
    if(player2_help->unload()){
#ifdef DEBUG
        qDebug()<<(tr("unload player2_help"));
#endif
    }
    if(player2_readme->unload()){
#ifdef DEBUG
        qDebug()<<(tr("unload player2_readme"));
#endif
    }
    if(player2_playlist->unload()){
#ifdef DEBUG
        qDebug()<<(tr("unload player2_playlist"));
#endif
    }
    if(player2_media_info->unload()){
#ifdef DEBUG
        qDebug()<<(tr("unload player2_media_info"));
#endif
    }
    delete ui;
#ifdef DEBUG
...
void Widget::on_media_info_clicked(){
    if(ui->media_info->isChecked()) {
        ...
        if(ui->playlistCheck->isChecked())
            setSize(settings->WIDGET->press_mus_but_width.toInt(),
                    settings->PLAYLIST->check_widgetHeight.toInt());
        else setSize(settings->WIDGET->press_mus_but_width.toInt(),
                     settings->PLAYLIST->not_check_widgetHeight.toInt());
    }
}
